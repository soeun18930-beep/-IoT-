...
MAIN_CSV = "voc_main_log.csv"
ADV_CSV  = "voc_advisory_log.csv"
OUT_PNG  = "voc_viz.png"
    ...
    with open(MAIN_CSV, "r", encoding="utf-8") as f:
        ...
        for r in rdr:
            try:
                ...
                continue
            xs.append(ts)
            y.append(float(r.get("value_ppb", "nan")))
            fh = r.get("forecast_ppb", None)
...
def main():
    ...
    if xs:
        plt.plot(xs,y,label="VOC (ppb)",color="tab:blue")
        if any(not math.isnan(v) for v in yhat):
            plt.plot(xs,yhat,"--",label="Forecast (ppb)",color="tab:orange")
        # 환기 ON 구간 음영
        seg_start=None
    ...
    for ev in evs:
        ...
        plt.text(mid,ymax*0.9,f"Advisory {ev['level']} (max {ev['max']:.0f} ppb)",
                 ha="center",va="top",fontsize=9,color=color)
    plt.title("VOC Trend & Advisory Intervals")
    plt.xlabel("Time"); plt.ylabel("ppb")
